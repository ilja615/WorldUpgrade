...
public class CobbledCliffsSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderConfig>
{
    ...
    private static final BlockState BOULDER = ModBlocks.BOULDER.get().defaultBlockState();
    ...
    {
        if (noise > 0.0f) {
            BlockPos.Mutable localMutable = new BlockPos.Mutable().set(x & 15, startHeight, z & 15);
            ...
            SurfaceBuilder.DEFAULT.apply(random, chunkIn, biomeIn, x, z, startHeight + cliffTallNess, noise, defaultBlock, defaultFluid, seaLevel, seed, CONFIG_STONE);
        } else {
