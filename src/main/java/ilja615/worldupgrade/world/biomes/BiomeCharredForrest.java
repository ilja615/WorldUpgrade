...
public class BiomeCharredForrest extends Biome
{
    public BiomeCharredForrest()
    { super((new Builder()).surfaceBuilder(new ConfiguredSurfaceBuilder(ModBiomeFeatures.CHARRED_F_SURFACE_BUILDER, SurfaceBuilder.GRAVEL_CONFIG)).precipitation(RainType.NONE).category(Category.PLAINS).depth(0.08F).scale(0.0F).temperature(0.4F).downfall(0.4F).waterColor(4159204).waterFogColor(329011).parent((String)null));
        ...
        this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, ModBiomeFeatures.DEAD_TREE.withConfiguration(DefaultBiomeFeatures.OAK_TREE_CONFIG).withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(4, 0.4F, 1))));
